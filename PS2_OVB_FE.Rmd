...
r_3int <- felm(data=data.final, aauw ~ ngirls + totchi + repub_i + female + ngirls*female + ngirls*totchi + ngirls*repub_i)


#model summaries
stargazer(r_fem, r_mal, r_1int, r_3int, type="text", header=FALSE, title="Comparison of Regressions", 
                     style="qje", 
...
View(data.final_fe)
r9 <- felm(data=data.final_fe, aauw ~ ngirls|year)
...
data.final <- fastDummies::dummy_cols(data.final, select_columns = "region")
...
FOR TEAM:I get the right coefficients but added all the dummy vars for region (when we should exclude one), not sure what's going on

```{r}
r10 <- felm(data=data.final, aauw ~ ngirls + female + white + repub_i + age + agesqrd + srvlng + srvlngsqrd + rel.none + rel.cath_orth + rel.othchris + rel.jew + demvote + totchi_0 + totchi_1 + totchi_2 + totchi_3 + totchi_4 + totchi_5 + totchi_6 + totchi_7 + totchi_9 + totchi_10 + region_1 + region_2 + region_3 + region_4 + region_5 + region_6 + region_7 + region_8 + region_9)

stargazer(r10, type="text", header=FALSE, title="Impact of Female Children on Legislator Voting on Women's Issues", 
                     style="qje", 
                     se = list(r10$rse),
                     omit = c("totchi_0","totchi_1", "totchi_2", "totchi_3", "totchi_4", "totchi_5", "totchi_6", "totchi_7", "totchi_9", "totchi_10", "region_1", "region_2", "region_3", "region_4", "region_5", "region_6", "region_7", "region_8", "region_9", "Constant"))
...
**Answer:** 
The region fixed effects are controlling for variation
